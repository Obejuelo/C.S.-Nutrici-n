...
function CoreNavHorizontalGroup(props)
{
                                            ...
                                            <ul className={clsx(classes.children, "popper-navigation-list", dense && "dense", "pl-0")}>
                                                {
                                                    item.children.map((item) => (
                                                        <React.Fragment key={item.id}>
                                                            ...
                                                            {item.type === 'collapse' && (
                                                                <NavHorizontalCollapse item={item} nestedLevel={nestedLevel} dense={dense}/>
                                                            )}

                                                            {item.type === 'item' && (
                                                                <NavHorizontalItem item={item} nestedLevel={nestedLevel} dense={dense}/>
                                                            )}

                                                            {item.type === 'link' && (
                                                                <NavHorizontalLink item={item} nestedLevel={nestedLevel} dense={dense}/>
                                                            )}
                                                        </React.Fragment>
                                                    ))
                                                }
...
CoreNavHorizontalGroup.propTypes = {
...
CoreNavHorizontalGroup.defaultProps = {};

const NavHorizontalGroup = withRouter(React.memo(CoreNavHorizontalGroup));
